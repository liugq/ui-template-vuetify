<template>
  <v-app>
    <!-- v-app-bar -->
    <v-app-bar app dense clipped elevate-on-scroll :color="appBar.color" :dark="appBar.img.display">
      <template v-slot:img>
        <v-img
          v-if="appBar.img.display"
          :src="appBarImgScr"
          height="100%"
          width="100%"
        ></v-img>
      ...
      <v-spacer></v-spacer>
      <v-btn icon @click.stop="null" title="帮助" class="hidden-xs-only">
      ...
      <v-btn icon @click="logout()" title="退出">
    ...
    <v-snackbar
      v-model="snackbarOption.snackbar"
    ...
    >
      ...
      <v-btn icon @click="snackbarOption.snackbar = false">
